...
# PROFESSIONAL SCALPING PARAMETERS (8+ Years Experience)
BASE_MARGIN_RATIO = 0.005  # Base position size (0.5% of balance)
MAX_MARGIN_RATIO = 0.015   # Maximum position size (1.5% of balance)
LEVERAGE = 125
ATR_MULTIPLIER = 2.0       # ATR multiplier for position sizing
MAX_PORTFOLIO_RISK = 0.02  # Maximum 2% portfolio risk at any time

# PROFESSIONAL CONFIDENCE THRESHOLDS
PREMIUM_CONFIDENCE = 0.85   # 85% for premium high-conviction trades
STANDARD_CONFIDENCE = 0.70  # 70% for standard trades
SCALPING_CONFIDENCE = 0.55  # 55% for scalping (more aggressive)
COUNTER_TREND_CONFIDENCE = 0.75  # 75% for counter-trend (higher due to risk)

# ADVANCED SCALPING CONFIGURATION
SCALPING_MODE_THRESHOLD = 0.70  # Activate scalping when confidence < 70%
SCALPING_MIN_INDICATORS = 4     # 4 out of 9 indicators must agree
SCALPING_MIN_ACTIVE = 5         # 5 indicators must be active
SCALPING_BASE_TP_ROI = 0.15     # Base 15% ROI (will be adjusted by volatility)
SCALPING_BASE_SL_ROI = -0.10    # Base 10% ROI stop loss (will be adjusted by volatility)

# VOLATILITY REGIME DETECTION
HIGH_VOLATILITY_THRESHOLD = 0.02  # 2% ATR threshold for high volatility
LOW_VOLATILITY_THRESHOLD = 0.005  # 0.5% ATR threshold for low volatility
VOLATILITY_LOOKBACK = 20         # Periods to look back for volatility calculation

# VOLUME CONFIRMATION REQUIREMENTS
MIN_VOLUME_RATIO = 1.2  # Minimum 20% above average volume
VOLUME_LOOKBACK = 20    # Periods for volume average calculation

# MULTI-TIMEFRAME MOMENTUM CONFIRMATION
MOMENTUM_TIMEFRAMES = ['1m', '5m', '15m']  # Timeframes for momentum confirmation
MIN_MOMENTUM_AGREEMENT = 2  # Minimum timeframes that must agree
...
# PROFESSIONAL TRADING UTILITIES (8+ Years Experience)

def calculate_atr(high, low, close, period=14):
    """Calculate Average True Range - Professional volatility measure"""
    try:
        tr1 = high - low
        ...
        return pd.Series(index=high.index, data=0.01)  # Fallback 1% ATR

def detect_volatility_regime(df, symbol):
    """Professional volatility regime detection"""
    try:
        atr = calculate_atr(df['high'], df['low'], df['close'], VOLATILITY_LOOKBACK)
        current_atr = atr.iloc[-1]
        price = df['close'].iloc[-1]
        atr_percentage = current_atr / price
        
        if atr_percentage >= HIGH_VOLATILITY_THRESHOLD:
            regime = 'HIGH'
            multiplier = 1.5  # Increase TP/SL in high volatility
        elif atr_percentage <= LOW_VOLATILITY_THRESHOLD:
            regime = 'LOW'
            multiplier = 0.7  # Decrease TP/SL in low volatility
        else:
            regime = 'MEDIUM'
            multiplier = 1.0
            
        return regime, atr_percentage, multiplier, current_atr
    except Exception as e:
        print(f"⚠️ {symbol}: Volatility regime detection error: {e}")
        return 'MEDIUM', 0.01, 1.0, 0.01

def calculate_professional_position_size(balance, current_price, atr, confidence, base_ratio=BASE_MARGIN_RATIO):
    """Professional ATR-based position sizing"""
    try:
        # Base position size
        base_size = balance * base_ratio
        
        # Adjust for confidence level
        confidence_multiplier = confidence / 0.70  # Normalize to 70% confidence
        confidence_multiplier = max(0.5, min(2.0, confidence_multiplier))  # Clamp between 0.5x and 2.0x
        
        # Adjust for volatility (inverse relationship)
        atr_percentage = atr / current_price
        volatility_multiplier = 1.0 / (1.0 + atr_percentage * 10)  # Reduce size in high volatility
        
        # Calculate final position size
        adjusted_size = base_size * confidence_multiplier * volatility_multiplier
        
        # Risk-based position sizing
        max_loss_per_trade = balance * 0.01  # Maximum 1% loss per trade
        stop_loss_distance = atr * ATR_MULTIPLIER
        max_position_by_risk = max_loss_per_trade / stop_loss_distance * current_price
        
        # Use the smaller of the two calculations
        final_size = min(adjusted_size, max_position_by_risk)
        
        # Ensure it doesn't exceed maximum
        max_size = balance * MAX_MARGIN_RATIO
        final_size = min(final_size, max_size)
        
        return final_size
    except Exception as e:
        print(f"⚠️ Position sizing error: {e}")
        return balance * BASE_MARGIN_RATIO

def analyze_volume_profile(df, symbol):
    ...
    try:
        ...
        volume_trend = df['volume'].tail(10).rolling(window=3).mean().diff().iloc[-1]
        ...
        if volume_ratio >= MIN_VOLUME_RATIO:  # Above average volume
            volume_score += 0.3
        
        if volume_trend > 0 and price_change > 0:  # Volume increasing with price
            volume_score += 0.3
        elif volume_trend > 0 and price_change < 0:  # Volume increasing against price (potential reversal)
            volume_score += 0.2
        
        if abs(price_change) > 0.001 and volume_ratio > 1.5:  # Significant move with high volume
            volume_score += 0.4
        ...
        print(f"⚠️ {symbol}: Volume profile error: {e}")
        return {'volume_ratio': 1.0, 'volume_trend': 0, 'volume_score': 0.5, 'volume_confirmation': False}

def calculate_support_resistance(df, lookback=20):
    """Calculate dynamic support and resistance levels"""
    try:
        high_prices = df['high'].tail(lookback)
        low_prices = df['low'].tail(lookback)
        
        # Pivot point calculation
        pivot = (high_prices.max() + low_prices.min() + df['close'].iloc[-1]) / 3
        
        # Support and resistance levels
        resistance_1 = 2 * pivot - low_prices.min()
        support_1 = 2 * pivot - high_prices.max()
        
        resistance_2 = pivot + (high_prices.max() - low_prices.min())
        support_2 = pivot - (high_prices.max() - low_prices.min())
        
        current_price = df['close'].iloc[-1]
        
        # Determine proximity to levels
        levels = [support_2, support_1, pivot, resistance_1, resistance_2]
        distances = [abs(current_price - level) / current_price for level in levels]
        nearest_level_distance = min(distances)
        ...
        return {
            ...
            'pivot': pivot,
            'nearest_distance': nearest_level_distance,
            'near_level': nearest_level_distance < 0.005  # Within 0.5%
        }
    except Exception as e:
        ...
        return {
            ...
            'support_2': current_price * 0.99,
            'resistance_1': current_price * 1.005,
            'resistance_2': current_price * 1.01,
            'pivot': current_price,
            'nearest_distance': 1.0,
            'near_level': False
        }

def analyze_multi_timeframe_momentum(symbol):
    """Professional multi-timeframe momentum confirmation"""
    try:
        momentum_signals = []
        timeframe_data = {}
        ...
        for timeframe in MOMENTUM_TIMEFRAMES:
            try:
                ...
                momentum = 0
                if current_rsi > 55 and current_macd > 0:
                    momentum = 1  # Bullish
                elif current_rsi < 45 and current_macd < 0:
                    momentum = -1  # Bearish
                ...
                print(f"⚠️ {symbol}: {timeframe} momentum error: {e}")
                momentum_signals.append(0)
                timeframe_data[timeframe] = {'rsi': 50, 'macd': 0, 'momentum': 0}
        
        # Analyze momentum agreement
        bullish_timeframes = sum(1 for m in momentum_signals if m == 1)
        bearish_timeframes = sum(1 for m in momentum_signals if m == -1)
        
        momentum_agreement = 'NEUTRAL'
        momentum_strength = 0.0
        
        if bullish_timeframes >= MIN_MOMENTUM_AGREEMENT:
            momentum_agreement = 'BULLISH'
            momentum_strength = bullish_timeframes / len(MOMENTUM_TIMEFRAMES)
        elif bearish_timeframes >= MIN_MOMENTUM_AGREEMENT:
            momentum_agreement = 'BEARISH'
            momentum_strength = bearish_timeframes / len(MOMENTUM_TIMEFRAMES)
        ...
        print(f"⚠️ {symbol}: Multi-timeframe momentum error: {e}")
        return {
            'momentum_agreement': 'NEUTRAL',
            'momentum_strength': 0.0,
            'timeframe_data': {},
...
def analyze_scalping_mode(symbol, entry_df, current_price, current_rsi, current_stoch_rsi_k, 
    ...
    try:
        ...
        if (buy_scalp_signals >= SCALPING_MIN_INDICATORS and 
            ...
            print(f"      Quick scalp: {buy_scalp_signals} BUY signals | Target: {SCALPING_TAKE_PROFIT_ROI*100:.0f}% ROI")
            ...
            print(f"      Quick scalp: {sell_scalp_signals} SELL signals | Target: {SCALPING_TAKE_PROFIT_ROI*100:.0f}% ROI")
...
def analyze_technical_indicators(symbol):
    """
    PROFESSIONAL 8+ YEARS EXPERIENCE SCALPING ANALYSIS
    
    Advanced multi-factor model with:
    - Volatility regime detection
    ...
    try:
        ...
        # Step 3: Professional volatility regime analysis
        ...
        if not volume_profile['volume_confirmation']:
            # Reduce confidence but don't skip entirely
            volume_penalty = -0.2
        else:
            volume_penalty = 0.0
        
        # Step 6: Support/Resistance analysis
        sr_levels = calculate_support_resistance(entry_df)
        ...
        # 2. RSI with volatility adjustment
        ...
        # 4. Bollinger Bands with volatility regime
        bb_upper, bb_lower, bb_mid = calculate_bollinger_bands(entry_df['close'])
        
        # 5. VWAP with institutional bias
        ...
        # PROFESSIONAL MULTI-FACTOR SIGNAL ANALYSIS
        signals = []
        confidence = 0.0
        indicator_status = {}
        ...
        # Session weighting (professional traders know when to trade)
        session_multiplier = 1.0
        if market_session['overlap']:
            session_multiplier = 1.3  # Best time to trade
        elif market_session['active']:
            session_multiplier = 1.1  # Good time to trade
        else:
            session_multiplier = 0.8  # Reduce confidence during inactive sessions
        
        # PROFESSIONAL TREND-FOLLOWING WITH CONFLUENCE ANALYSIS
        if trend_direction == 'BULLISH':
            # LONG SIGNALS ONLY - Professional weighting based on 8+ years experience
            
            # 1. MACD Signal (20% weight) - Most important for momentum
            macd_strength = abs(current_macd_histogram) * 1000  # Normalize
            if current_macd_line > current_macd_signal and current_macd_histogram > 0:
                weight = 0.20 + min(0.05, macd_strength * 0.01)  # Bonus for strong momentum
                signals.append('BUY')
                confidence += weight
            ...
            # 2. RSI Signal (18% weight) - Volatility adjusted levels
            rsi_threshold = RSI_EXTREME_OVERSOLD
            if volatility_regime == 'HIGH':
                rsi_threshold = 25  # Higher threshold in volatile markets
            elif volatility_regime == 'LOW':
                rsi_threshold = 15  # Lower threshold in calm markets
                
            if current_rsi < rsi_threshold:
                weight = 0.18 + (rsi_threshold - current_rsi) * 0.002  # Bonus for extreme levels
                signals.append('BUY')
                confidence += weight
            ...
            # 4. Support/Resistance Confluence (15% weight)
            if sr_levels['near_level'] and current_price <= sr_levels['support_1'] * 1.002:
                signals.append('BUY')
                confidence += 0.15
                indicator_status['SR_CONFLUENCE'] = 'BUY'
            elif current_price > sr_levels['pivot']:
                signals.append('BUY')
                confidence += 0.08  # Reduced weight above pivot
                indicator_status['SR_CONFLUENCE'] = 'BUY'
            else:
                indicator_status['SR_CONFLUENCE'] = 'NEUTRAL'
            
            # 5. Volume Profile Confirmation (12% weight)
            if volume_profile['volume_confirmation']:
                weight = 0.12 * volume_profile['volume_score']
                signals.append('BUY')
                confidence += weight
                indicator_status['VOLUME'] = 'BUY'
            else:
                indicator_status['VOLUME'] = 'NEUTRAL'
            
            # 6. VWAP Institutional Bias (8% weight)
            vwap_distance = (current_price - current_vwap) / current_vwap
            if vwap_distance > -0.005:  # Within 0.5% of VWAP or above
                signals.append('BUY')
                confidence += 0.08
            ...
            # 8. Bollinger Band Mean Reversion (3% weight) - Lower weight for mean reversion in trend
            bb_position = (current_price - current_bb_lower) / (current_bb_upper - current_bb_lower)
            ...
            # 9. Supertrend Confirmation (2% weight) - Least important
            ...
            # SHORT SIGNALS ONLY - Professional weighting based on 8+ years experience
            ...
            # 2. RSI Signal (18% weight) - Volatility adjusted levels
            rsi_threshold = RSI_EXTREME_OVERBOUGHT
            if volatility_regime == 'HIGH':
                rsi_threshold = 75  # Lower threshold in volatile markets
            elif volatility_regime == 'LOW':
                rsi_threshold = 85  # Higher threshold in calm markets
                
            if current_rsi > rsi_threshold:
                weight = 0.18 + (current_rsi - rsi_threshold) * 0.002  # Bonus for extreme levels
                signals.append('SELL')
                confidence += weight
            ...
            # 4. Support/Resistance Confluence (15% weight)
            if sr_levels['near_level'] and current_price >= sr_levels['resistance_1'] * 0.998:
                signals.append('SELL')
                confidence += 0.15
                indicator_status['SR_CONFLUENCE'] = 'SELL'
            elif current_price < sr_levels['pivot']:
                signals.append('SELL')
                confidence += 0.08  # Reduced weight below pivot
                indicator_status['SR_CONFLUENCE'] = 'SELL'
            else:
                indicator_status['SR_CONFLUENCE'] = 'NEUTRAL'
            ...
            if volume_profile['volume_confirmation']:
                ...
                signals.append('SELL')
                confidence += weight
                indicator_status['VOLUME'] = 'SELL'
            else:
                indicator_status['VOLUME'] = 'NEUTRAL'
            
            # 6. VWAP Institutional Bias (8% weight)
            vwap_distance = (current_price - current_vwap) / current_vwap
            if vwap_distance < 0.005:  # Within 0.5% of VWAP or below
                signals.append('SELL')
                confidence += 0.08
        ...
        # Collect all indicator values for detailed analysis - PROFESSIONAL VERSION
        indicator_values = {
        ...
        # PROFESSIONAL DECISION LOGIC - 8+ YEARS EXPERIENCE
        buy_signals = signals.count('BUY')
        sell_signals = signals.count('SELL')
        total_active_indicators = buy_signals + sell_signals
        
        # Professional confidence thresholds based on market conditions
        required_confidence = STANDARD_CONFIDENCE
        if volatility_regime == 'HIGH':
            required_confidence = PREMIUM_CONFIDENCE  # Higher threshold in volatile markets
        elif market_session['overlap']:
            required_confidence = SCALPING_CONFIDENCE  # Lower threshold during best time
        
        # Professional requirements - Adaptive based on market conditions
        min_indicators = 4 if volatility_regime == 'HIGH' else 3
        min_active = 6 if market_session['active'] else 5
        
        if (buy_signals >= min_indicators and 
            buy_signals > sell_signals and 
            confidence >= required_confidence and
            total_active_indicators >= min_active):
            
            # Record trade time
            LAST_TRADE_TIME[symbol] = time.time()
            return 'BUY', confidence, indicator_values
            
        elif (sell_signals >= min_indicators and 
...
def trade():
    ...
    print(f"   ⚖️  ATR-Based Position Sizing Active")
    ...
    for i in range(0, len(SYMBOLS), BATCH_SIZE):
        ...
        for symbol_index, symbol in enumerate(batch):
            try:
                ...
                # PROFESSIONAL POSITION SIZING - ATR Based
                precision = get_quantity_precision(symbol)
                min_quantity = get_min_quantity(symbol)
                
                # Get professional position size based on ATR and confidence
                current_atr = indicators.get('current_atr', current_price * 0.01)  # Fallback 1% ATR
                professional_position_size = calculate_professional_position_size(
                    usdt_balance, current_price, current_atr, confidence, BASE_MARGIN_RATIO
                )
                
                # Convert to quantity
                quantity = round(professional_position_size * current_leverage / current_price, precision)
                ...
                position_notional = quantity * current_price / current_leverage
                portfolio_risk = position_notional / usdt_balance
                
                if portfolio_risk > MAX_PORTFOLIO_RISK:
                    quantity = round((usdt_balance * MAX_PORTFOLIO_RISK * current_leverage) / current_price, precision)
                    print(f"   ⚠️  Position size reduced due to portfolio risk limit")
                
                volatility_regime = indicators.get('volatility_regime', 'MEDIUM')
                atr_pct = indicators.get('atr_percentage', 0.01) * 100
                ...
                if signal == 'BUY':
                    ...
                    if is_scalping_mode:
                        ...
                        print(f"🚀 {symbol}: PROFESSIONAL LONG{session_warning}")
                        print(f"   📈 Trend: {trend_direction} (Strength: {trend_strength:.1f}) | Vol: {volatility_regime}")
                        print(f"   🎯 Confidence: {confidence:.1%} (ATR: {atr_pct:.2f}%) | Indicators: {buy_count}✅ {sell_count}❌ {neutral_count}⚪")
                        print(f"   🟢 Agreeing: {', '.join(buy_indicators)}")
                        print(f"   💎 RSI: {indicators.get('rsi', 0):.1f} | MACD: {indicators.get('macd_histogram', 0):.4f}")
                        print(f"   📊 MTF Momentum: {indicators.get('mtf_momentum', {}).get('momentum_agreement', 'N/A')} | Volume: {indicators.get('volume_profile', {}).get('volume_confirmation', False)}")
                        print(f"   💰 Position: ${professional_position_size:.2f} (Risk: {portfolio_risk:.2%})")
                    ...
                    if is_scalping_mode:
                        ...
                        print(f"🚀 {symbol}: PROFESSIONAL SHORT{session_warning}")
                        print(f"   📉 Trend: {trend_direction} (Strength: {trend_strength:.1f}) | Vol: {volatility_regime}")
                        print(f"   🎯 Confidence: {confidence:.1%} (ATR: {atr_pct:.2f}%) | Indicators: {buy_count}✅ {sell_count}❌ {neutral_count}⚪")
...
print("🚀 PROFESSIONAL CRYPTO FUTURES TRADING SYSTEM")
print("="*60)
print("🎯 8+ YEARS EXPERIENCE SCALPING BOT")
print("   • ATR-Based Professional Position Sizing")
print("   • Multi-Timeframe Momentum Confirmation (1m→5m→15m)")
print("   • Volatility Regime Detection & Adaptation") 
print("   • Volume Profile & Institutional Flow Analysis")
print("   • Support/Resistance Confluence Trading")
print("   • Professional Risk Management (2% max portfolio risk)")
print("   • Market Microstructure Considerations")
print("   • Session-Weighted Confidence Scoring")
print("="*60)
print()
...
print("✅ All professional validations passed")
print("⚡ ADAPTIVE CONFIDENCE: 55% overlap | 70% standard | 85% high-volatility") 
print("🚀 Beginning professional scalping operations...")
print("📊 ATR-Based Sizing | MTF Momentum | Volume Confirmation")
print("="*60)
print()
...
while True:
    try:
        trade()
        print(f"⏱️  Professional cycle complete. Next analysis in 30 seconds...")
        time.sleep(30)  # Professional scalping frequency
    except Exception as e:
